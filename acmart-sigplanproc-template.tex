...
There has been a wealth of work done in the space of visualizing data-flow analysis, which can be considered a superset of taint-flow analysis. We highlight some related data-flow visualization efforts. There has been less work specifically on visualizing taint-flow, so we more thoroughly discuss efforts in this specific domain.
\newline\newline
...
\newline\newline
\indent \textbf {"VisuFlow"} is a plugin developed for the Eclipse IDE that generates visualizations for static analysis performed by Soot. While similar to our approach, their plugin does not work with CodeQL queries, and was developed for a different IDE.
\[TODO\] 
